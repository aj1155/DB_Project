...
/*mutipart/form-data 처리를 위한 미들웨어*/
...
router.post('/search', function (req, res, next) {
    console.error("search " + JSON.stringify(req.body));
    var content = req.body.content;
    var result = true;
    var message = "";
    ...
    if (!isNaN(content) || !isNaN(content.substring(0, content.length - 1))) {
        ...
        if (!isNaN(content) == true) {
            ...
            //~기로 친경우
            grade = content.substring(0, content.length - 1);
        }

        //모든 기수 다 가져오기
        userDao.selectUserGradeAll(grade, category_id, function (row) {
            console.error("rows " + JSON.stringify(row));
            rows = row;
            result = true;
            return res.send({
                result: result,
    ...
    User.create({
        ...
        .then(function (result) {
            ...
            User.findAll({
                ...
                .then(function (rows) {
                    res.render('admin/userManage', {
                        userList: rows, msg: "정상적으로 회원가입을 하였습니다.", type: "success",
            ...
            exUd.upload(req, res, function (err) {
                ...
                exTJ.exToJson(req, function (result) {
                    if (result.msg == 'excelTypeErr') {
                        res.render('admin/userManage', {
                            userList: list,
                            msg: "엑셀형식을 확인해주세요 ms excel을 이용해야 합니다.",
                            type: "error",
                            srchType: 0,
                            srchText: "",
                            count: 10
